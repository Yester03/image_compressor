...
![image-20260409003930584](./assets/image-20260409003930584.png)

一个面向本地批处理的图片工具，提供两类核心能力：
- JPEG 目标体积压缩（按 KB 目标自动质量/尺寸折中）
- 像素风格化（拼豆风格）转换

当前版本：`v1.0.0`（稳定首发版，功能聚焦“压缩 + 像素风格化”）
...
- `<input>`：图片文件或目录
- `--mode`：`compress` 或 `perler`（默认 `compress`）
...
- Windows 拖拽模式：将图片拖到 `image-compressor-*.exe` 上，程序会自动进入拖拽处理流程。
